<?php
function racecyclops_getmoduleinfo(){
	$info = array(
		"name"=>"Race - Cyclops",
		"version"=>"1.2",
		"author"=>"`4Thanatos",
		"category"=>"Races",
		"settings"=>array(
			"Cyclops Race Settings,title",
			"turnsup"=>"Extra Turns's,int|0",
			"pvpup"=>"Extra PVP's,int|0",
      "buffname"=>"Buff Name,text|`@Cyclopsish Might`0",
      ...
      "minedeathchance"=>"Chance for Cyclopss to die in the mine,range,0,100,1|0",
      "dk_req"=>"How many DKs do you need before the race is available?,int|0",
      "racename"=>"Race Name :,hidden|Cyclops",
      ...
      "levelhpinc"=>"each level increases hp(cur) by X:,int|40",
      ...
      "cre_req"=>"Victories Required to level up the race(0 to disable),int|130",
		),
		"prefs"=>array(
...
function racecyclops_uninstall(){
	global $session;
	$sql = "UPDATE  " . db_prefix("accounts") . " SET race='" . RACE_UNKNOWN . "' WHERE race='Cyclops '";
	db_query($sql);
	if ($session['user']['race'] == 'Cyclops')
		$session['user']['race'] = RACE_UNKNOWN;
	return true;
}
function racecyclops_dohook($hookname,$args){
  ...
  break;
	...
	case "chooserace":
	  ...
	  if(get_module_pref("race_on")&&
       $session[user][dragonkills]>$dk_req){
		output("<a href='newday.php?setrace=$race$resline'>`0Within the deepest cave of %s</a>, dwell the massive`5Cyclops`0 one-eyed gigantic monsters`n`n", $city, true);
		addnav("`#Cyclops`0","newday.php?setrace=$race$resline");
		addnav("","newday.php?setrace=$race$resline");
		}
		break;
	case "setrace":
		if ($session['user']['race']==$race){
			output("`#As a Cyclops, you are a giant one-eyed monster.`n");
	...
	case "newday":
		if ($session['user']['race']==$race){
			racecyclops_checkcity($module);
			racecyclops_applystats($module);
		}
		break;
  }
  	return $args;
}
function racecyclops_applystats($module){
  global $session;
...
function racecyclops_checkcity(){
  global $session;
  $module="racecyclops";
