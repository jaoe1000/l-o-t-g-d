<?php
	...
	// edited & enhanced by Fieser-Kardinal
	//
	// Option to define max. Number of Officers/Leaders allowed in Clan
...
/*//*/				$args = modulehook("clan-setrank", array("setrank"=>$setrank, "login"=>$who, "name"=>$whoname, "acctid"=>$whoacctid));
	/*//*/			if (!(isset($args['handled']) && $args['handled'])) {
		...
		while ($row=db_fetch_assoc($result)){
			...
			if ($row['clanrank']>=CLAN_OFFICER){
				rawoutput("<td>");
				if ($countoff<=$offmax && $row['clanrank']<$session['user']['clanrank'] && $row['clanrank']<30){
					rawoutput("<a href='clan.php?op=membership&setrank=".clan_nextrank($ranks,$row['clanrank'])."&who=".rawurlencode($row['login'])."&whoname=".rawurlencode($row['name'])."&whoacctid=".$row['acctid']."'>$promote</a><br>");
					...
					output_notl("`)%s`0`n", $promote);
				}	
				if ($row['clanrank']<=$session['user']['clanrank'] && $row['clanrank']>CLAN_APPLICANT && $row['login']!=$session['user']['login']){
					rawoutput("<a href='clan.php?op=membership&setrank=".clan_previousrank($ranks,$row['clanrank'])."&whoacctid=".$row['acctid']."'>$demote</a><br>");
				...
				if ($row['clanrank'] <= $session['user']['clanrank'] && $row['login']!=$session['user']['login']){
					...
					output_notl("`)%s`0`n", $remove);
				}
				rawoutput("</td>");				
			}else{
				rawoutput("<td>");
				if($countoff<=$offmax && $row['clanrank']<$session['user']['clanrank'] && $row['clanrank']<19){
					rawoutput("<a href='clan.php?op=membership&setrank=".clan_nextrank($ranks,$row['clanrank'])."&who=".rawurlencode($row['login'])."&whoname=".rawurlencode($row['name'])."&whoacctid=".$row['acctid']."'>$promote</a><br>");
					addnav("","clan.php?op=membership&setrank=".clan_nextrank($ranks,$row['clanrank'])."&who=".rawurlencode($row['login'])."&whoname=".rawurlencode($row['name'])."&whoacctid=".$row['acctid']);
				}elseif($countoff<=$offmax-1 && $row['clanrank']<$session['user']['clanrank'] && $row['clanrank']==19){
					rawoutput("<a href='clan.php?op=membership&setrank=".clan_nextrank($ranks,$row['clanrank'])."&who=".rawurlencode($row['login'])."&whoname=".rawurlencode($row['name'])."&whoacctid=".$row['acctid']."'>$promote</a><br>");
					addnav("","clan.php?op=membership&setrank=".clan_nextrank($ranks,$row['clanrank'])."&who=".rawurlencode($row['login'])."&whoname=".rawurlencode($row['name'])."&whoacctid=".$row['acctid']);
				}else{
					output_notl("`)%s`0`n", $promote);
		...
		rawoutput("</table>");
		output("`n`^This clan has a total of `\$%s`^ dragon kills.",$tot);
?>