...
// 8th October 2004
// Modified for Glorfindal 4/17/2005 by Niksolo
...
function elvinbank_getmoduleinfo(){
	$info = array(
		"name"=>"Elvin Bank",
		"version"=>"2.0",
		"author"=>"E Stevens, JT Traub, S Brown, adapted for Elvin City by Niksolo",
		"category"=>"Village",
		"download"=>"http://dragonprime.net/users/Finwe/elvinbank.zip",
		"settings"=>array(
		...
		"prefs"=>array(
			"giventoday"=>"Has the user given a donation today?,bool|0",
		)
	);
	return $info;
}

function elvinbank_install(){
...
function elvinbank_uninstall(){
	return true;
}

function elvinbank_dohook($hookname,$args){
	global $session;
	switch($hookname){
	case "newday":
		set_module_pref("giventoday",0,"elvinbank");
		break;
	case "changesetting":
...
function elvinbank_run(){
	global $session;
	$price= (($session['user']['level']*($session['user']['dragonkills']+1))+50);
	$op = httpget("op");
	$allowdep=get_module_setting("allowdep");
	$allowtx=get_module_setting("allowtx");
	$giventoday=get_module_pref("giventoday");
	page_header("Finwe's Bank");
	output("`#`c`bF`3inwe's `#B`3ank`b`c");
	$op = httpget('op');
	if ($op==""){
		checkday();
		output("`7You enter the ancient wooden structure.  You notice `#F`3inwe `7behind a desk.`n`n");
		output("`3\"`&Greetings, %s,`3\" `#F`3inwe `7says with a warm smile.",$session['user']['sex']?"Madam":"Sir");
	...
	}elseif($op=="give" && $giventoday==0 && $session['user']['goldinbank']>=$price){
		$session['user']['goldinbank']-=$price;
		output("`7You inform `#F`3inwe `7that you'd like to give a donation from your account.");
		output("`7He smiles warmly at your thought of your donation.`n`n");
		output("`7He calls his assistant.  A small light approaches and a form appears in front of you .");
		output("`7You look at it and decide that donating is the right and noble thing to do.");
		output("`7You finish the form, handing your donation over.`n`n");
		output("`7You feel really good about yourself!");
		apply_buff('elvinbank',array("name"=>"Generosity","rounds"=>20,"defmod"=>1.02));
		set_module_pref("giventoday",1);
	}elseif($op=="give" && $giventoday==0 && $session['user']['goldinbank']<$price){
		output("`7You inform `#F`3inwe `7that you'd like to donate some money from your account.");
		output("`#F`3inwe `7 softly smiles, but says that you do not have sufficient funds.`n`n");
		output("Perhaps you'll donate another time when you have more money.");
	}elseif($op=="give" && $giventoday==1){
		output("`7You inform `#F`3inwe `7that you'd like to donate some money from your account.");
		output("`#F`3inwe `7 softly smiles, but says that your generosity is kind and has already helped many.`n`n");
		output("Perhaps you'll donate another time.");
	}elseif($op=="transfer" && $allowtx){
		output("`7`bTransfer Money`b:`n");
		if ($session['user']['goldinbank']>=0){
			output("`7`#F`3inwe `7 tells you, \"`&You understand of course, you may only transfer `^%s`& gold for each level that the recipient has achieved.",getsetting("transferperlevel",25));
			...
			rawoutput("<form action='runmodule.php?module=elvinbank&op=transfer2' method='POST'>");
			...
			addnav("","runmodule.php?module=elvinbank&op=transfer2");
		}else{
			output("`7\"`7I'm sorry, I can't allow you to transfer to someone who is already in our debt, I hope you understand.`7\"");
		}
		...
		if (db_num_rows($result)==1){
			...
			rawoutput("<form action='runmodule.php?module=elvinbank&op=transfer3' method='POST'>");
			...
			addnav("","runmodule.php?module=elvinbank&op=transfer3");
		}elseif(db_num_rows($result)>100){
			output("`7`#F`3inwe `7 chuckles at you, and suggests that a search that broad would take all day, and that perhaps you could narrow it down for him a little.`n`n");
			...
			output("`#F`3inwe's `7eyes sparkle as he smiles widely, \"`&Perhaps you'd care to try someone who's one of our customers, since I can't seem to find that name in my records.`7\"");
		}
		...
		if ($session['user']['gold']+$session['user']['goldinbank']<$amt){
			output("`7`#F`3inwe `7 shakes his head with a smile, \"`&How can you transfer `^%s`& gold when our bank holds only  `^%s`& for you?`7\"",$amt,$session['user']['gold']+$session['user']['goldinbank']);
		}else{
			...
			if (db_num_rows($result)==1){
				...
				if ($session['user']['amountouttoday']+$amt > $maxout) {
					output("`7`#F`3inwe `7 smiles slightly, \"`&Perhaps I did not explain clearly? We ask our customers to transfer no more than `^%s`& gold total per day.`7\"",$maxout);
				}else if ($maxtfer<$amt){
					output("`7`#F`3inwe `7 smiles slightly, \"`&Perhaps I did not explain clearly? `&%s`& may only receive up to `^%s`& gold per day.`7\"",$row['name'],$maxtfer);
				}else if($row['transferredtoday']>=getsetting("transferreceive",3)){
					output("`7`#F`3inwe `7 smiles slightly, \"`&Perhaps I did not explain clearly? `&%s`& has received too many transfers today, perhaps you will try tomorrow.`7\"",$row['name']);
				}else if($amt<(int)$session['user']['level']){
					output("`7`#F`3inwe `7 smiles slightly, \"`&Perhaps I did not explain clearly? \"`&We ask all our customers to transfer at least as much as their level.`7\"");
				}else if($row['acctid']==$session['user']['acctid']){
					output("`7`#F`3inwe `7 smiles slightly, \"`&I'd just as soon not transfer money from yourself to yourself. It's rather a silly waste of my time, wouldn't you agree?`7\"");
				}else{
					...
					db_query($sql);
					output("`7`#F`3inwe `7 smiles, \"`&The transfer has been completed!`7\"");
				...
				output("`7`#F`3inwe `7 looks up from his book and apologizes, \"`&I am sorry, I don't believe I caught that. Could you tell me again what you would like to transfer?`7\"");
			}
		}
	}elseif($op=="deposit" && $allowdep){
		output("`0");
		rawoutput("<form action='runmodule.php?module=elvinbank&op=depositfinish' method='POST'>");
		$balance = translate_inline("`7`#F`3inwe `7 says, \"`&You have a balance of `^%s`& gold in the bank.`7\"`n");
		$debt = translate_inline("`7`#F`3inwe `7 says, \"`&You have a `\$debt`& of `^%s`& gold to the bank.`7\"`n");
		...
		addnav("","runmodule.php?module=elvinbank&op=depositfinish");
	}elseif($op=="depositfinish" && $allowdep){
		$amount = abs((int)httppost('amount'));
		if ($amount==0){
			$amount=$session['user']['gold'];
		}
		$notenough = translate_inline("`\$ERROR: Not enough gold in hand to deposit.`n`n`^You plunk your `&%s`^ gold on the counter and declare that you would like to deposit all `&%s`^ gold of it.`n`n`7`#F`3inwe `7 laughs and suggests you recount your money.");
		$depositdebt = translate_inline("`7`#F`3inwe `7 records your deposit of `^%s `7gold in the book before him. \"`&Thank you, `&%s`&.  You now have a debt of `\$%s`& gold to the bank and `^%s`& gold in hand.`7\"");
		$depositbalance= translate_inline("`7`#F`3inwe `7 records your deposit of `^%s `7gold in the book before him. \"`&Thank you, `&%s`&.  You now have a balance of `^%s`& gold in the bank and `^%s`& gold in hand.`7\"");
		...
		$balance = translate_inline("`7`#F`3inwe `7 scans through the book before him, \"`&You have a balance of `^%s`& gold in the bank.`7\"`n");
		$debt = translate_inline("`7`#F`3inwe `7 scans through the book before him, \"`&You have a `\$debt`& of `^%s`& gold to the bank.`7\"`n");
		rawoutput("<form action='runmodule.php?module=elvinbank&op=withdrawfinish' method='POST'>");
		...
		addnav("","runmodule.php?module=elvinbank&op=withdrawfinish");
	}elseif($op=="withdraw"){
		$withdraw = translate_inline("Withdraw");
		$balance = translate_inline("`7`#F`3inwe `7 scans through the book before him, \"`&You have a balance of `^%s`& gold in the bank.`7\"`n");
		$debt = translate_inline("`7`#F`3inwe `7 scans through the book before him, \"`&You have a `\$debt`& of `^%s`& gold in the bank.`7\"`n");
		rawoutput("<form action='runmodule.php?module=elvinbank&op=withdrawfinish' method='POST'>");
		output($session['user']['goldinbank']>=0?$balance:$debt,abs($session['user']['goldinbank']));
		...
		if ($amount>$session['user']['goldinbank'] && httppost('borrow')=="") {
			...
			output("`#F`3inwe `7 chuckles at you and suggests you rethink your request. You realize your mistake and think you should try again.");
			...
			if ($lefttoborrow<=$session['user']['goldinbank']+$maxborrow){
				...
				if ($lefttoborrow-$session['user']['goldinbank'] > $maxborrow){
					...
					output("`#F`3inwe `7 looks up your account and informs you that you may only borrow up to `^%s`7 gold.", $maxborrow);
				}else{
					if ($didwithdraw) {
					...
					output("`7`#F`3inwe `7 records your withdrawal of `^%s `7gold in the book before him. \"`&Thank you, `&%s`&.  You now have a debt of `\$%s`& gold to the bank and `^%s`& gold in hand.`7\"", $amount,$session['user']['name'], abs($session['user']['goldinbank']),$session['user']['gold']);
				}
			}else{
				output("`7Considering the `^%s`7 gold in your account, you ask to borrow `^%s`7. `#F`3inwe `7looks up your account and informs you that you may only borrow up to `^%s`7 gold at your level.", $session['user']['goldinbank'], $lefttoborrow-$session['user']['goldinbank'], $maxborrow);
			}
		}else{
			$session['user']['goldinbank']-=$amount;
			$session['user']['gold']+=$amount;
			debuglog("withdrew $amount gold from the bank");
			output("`7`#F`3inwe `7 records your withdrawal of `^%s `7gold in the book before him. \"`&Thank you, `&%s`&.  You now have a balance of `^%s`& gold in the bank and `^%s`& gold in hand.`7\"", $amount,$session['user']['name'], abs($session['user']['goldinbank']),$session['user']['gold']);
		}
	}
	villagenav();
	addnav("Money");
	if ($session['user']['goldinbank']>=0){
		addnav("W?Withdraw","runmodule.php?module=elvinbank&op=withdraw");
		if ($allowdep)
			addnav("D?Deposit","runmodule.php?module=elvinbank&op=deposit");
		if (getsetting("borrowperlevel",20))
			addnav("L?Take out a Loan","runmodule.php?module=elvinbank&op=borrow");
	}else{
		if ($allowdep)
			addnav("D?Pay off Debt","runmodule.php?module=elvinbank&op=deposit");
		if (getsetting("borrowperlevel",20))
			addnav("L?Borrow More","runmodule.php?module=elvinbank&op=borrow");
	}
	if ($allowtx){
		if ($session['user']['level']>=getsetting("mintransferlev",3) ||
				$session['user']['dragonkills']>0){
			addnav("M?Transfer Money","runmodule.php?module=elvinbank&op=transfer");
		}
	}
	addnav("Give Donation","runmodule.php?module=elvinbank&op=give");
	page_footer();
}
