...
$i = 0;
$status_map = [
    1 => translate_inline("`#Occupied"),
    2 => translate_inline("`@Financing"),
    3 => translate_inline("`QIn Construction"),
    4 => translate_inline("`!Abandoned"),
    5 => translate_inline("`%For Sale")
];
...
while($row = db_fetch_assoc($result)){
    ...
    // Get the status from our array, default to empty string if not found
    $status = $status_map[$row['status']] ?? translate_inline("Unknown");
    ...
    $status = $stat['status'];
    $name = ($row['name'] == "") ? translate_inline("Unnamed") : $row['name'];
    
    output_notl($name);
    rawoutput("</td><td>");
    
    $result2 = db_query("SELECT name FROM ".db_prefix("accounts")." WHERE acctid='{$row['ownerid']}'");
    if (db_num_rows($result2) > 0) {
        $row2 = db_fetch_assoc($result2);         
        output_notl($row2['name']);
    } else {
        output_notl(translate_inline("Abandoned"));
    }
